#!/usr/bin/env python3
"""Build the #87 adjudication material from items87.json (the #57/#63/#85 pattern, re-pointed at the second spot check).

  python3 build87.py sheet                      -> third-opinion87.md (next to this file)
  python3 build87.py page VIEWS_DIR OUT.html    -> the adjudication page, views inlined (the #57 template, retitled)

items87.json comes from dump2json63.py (the spot check's dump parsed) with the third-opinion fields filled by the
reading; adjudicated87.json, when present, holds Daniel's verdicts read from the page's store. Field names follow
...
ITEMS = json.load(open(os.path.join(HERE, "items87.json")))
ADJ_PATH = os.path.join(HERE, "adjudicated87.json")
...
def sheet():
    ...
    out.append("")
    open(os.path.join(HERE, "third-opinion87.md"), "w").write("\n".join(out) + "\n")
...
def page(views_dir, out_path):
    ...
    tpl = (tpl.replace("<title>The 69 Disagreements</title>", "<title>The Second Spot Check</title>")
              .replace("<h1>The 69 Disagreements</h1>", f"<h1>The Second Spot Check: {len(ITEMS)} Disagreements</h1>")
              .replace("qwen3.8-27b-nvfp4 vs Claude Sonnet 4.6, thinking off, the 125 under", f"the second re-rating batch (qwen3.8-27b-nvfp4, thinking off) vs Claude Sonnet 4.6 (thinking off) on {n_ex} sampled rows under")
              .replace("adjudication for #57", "adjudication for #87")
              .replace('<span id="progTxt">0 of 69</span>', f'<span id="progTxt">0 of {len(ITEMS)}</span>')
              .replace('const LS_KEY = "adjudication-57-verdicts";', 'const LS_KEY = "adjudication-87-verdicts";')
